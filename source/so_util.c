/* Android ARM64 shared-object loader.
 *
...
void so_finalize(so_module *mod) {
  ...
  // Set executable pages before writable pages; Horizon disallows W-to-X.
  ...
  // Symbol tables must follow the live code-memory mapping after finalization.
...
// Emit a /proc/self/maps-style listing of loaded modules.
...
void so_execute_init_array(so_module *mod) {
  for (int i = 0; i < mod->elf_hdr->e_shnum; i++) {
    ...
    if (strcmp(sh_name, ".init_array") == 0) {
      ...
      int n = (int)(mod->sec_hdr[i].sh_size / sizeof(*init_array));
      for (int j = 0; j < n; j++) {
        if (init_array[j] != 0)
          init_array[j]();
      }
...
// Layout expected by bionic/libunwind.
